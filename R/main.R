...
#' @useDynLib numbat.custom, .registration=TRUE
NULL
...
run_numbat = function(
        ...
        init_clusters = NULL, cells_per_subcluster = 100
    ) {
    ...
    ######### Log parameters #########
    log_message(paste('\n',
        glue('numbat.custom version: ', as.character(utils::packageVersion("numbat.custom"))),
        ...
        glue('plot = {plot}'),
        glue('genome = {genome}'),
        glue('init_clusters = {ifelse(is.null(init_clusters), "None", paste0(length(unique(init_clusters)), " clusters"))}'),
        glue('cells_per_subcluster = {cells_per_subcluster}'),
        'Input metrics:',
    ...
    # When init_clusters is provided, use all subtrees; otherwise use init_k
    effective_k <- if (!is.null(init_clusters)) length(subtrees) else init_k
    clones = purrr::keep(subtrees, function(x) x$sample %in% as.character(1:effective_k))
    ...
    for (i in 1:max_iter) {
        ...
        # Skip size filtering when init_clusters is used (user-defined clustering)
        ...
        # diagnostics (skip when using init_clusters as there's no root sample 0)
        if (i == 1 && is.null(init_clusters)) {
